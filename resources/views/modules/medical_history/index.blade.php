...
<div class="content container ">
    <div class="col-sm-12">
        <div class="row">
            <div class="col-sm-6 card px-2 p-2 rounded d-flex align-items-center">
                <div class="col-sm-10 card px-2 p-2 rounded">
                    <button type="button" class="btn btn-secondary btn-lg" data-toggle="modal" data-target="#staticBackdrop_two"><i class="fas fa-edit"></i> {{ __('Cliente Antiguo') }}</button>
                </div>
            </div>
            <div class="col-sm-6 card px-2 p-2 rounded d-flex align-items-center">
                <div class="col-sm-10 card px-2 p-2 rounded">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <select style="border-radius:10px;margin: 0% 0% 0% 1%;" class="btn btn-light" aria-label="Default select example" name="type_gender_pet" required>
                                    <option value="">--Seleccione el genero--</option>
                                    <option value="{{ __('FEMALE_') }}">{{ __('FEMALE_') }}</option>
                                    <option value="{{ __('MALE_') }}">{{ __('MALE_') }}</option>
                                </select>
                            </div>
                        </div>
        ...
        <!-- FinModal -->
        <!-- Modal Old-->
        <div class="modal fade" id="staticBackdrop_two" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="modal-title" id="staticBackdropLabel">Nueva Mascota</h3>
                        <button type="button" class="close" data-dismiss="modal_old" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>

                    <form class="formulario-send_request" action="{{ route('medical_history.create_old') }}" method="POST">
                        @csrf
                        <div class="modal-body">
                            <div class="text-center">
                                <h5>{{ __('OWNER_DATA') }}</h5>
                            ...
                            <div class="form-group">
                                <label for="asunto">{{ __('DOCUMENT') }}:</label>
                                <input class="p-1 mb-2 bg-white text-dark form-control" style="border: none;outline: none;border-bottom: 2px solid #717171;" required maxlength="45" type="text" id="document_customer_old" name="document_customer_old" placeholder="Documento del propietario" oninput="this.value = this.value.toUpperCase().replace(/[^A-Z- 0-9]/,'')" required>
                            </div>
                            <hr>
                            <div class="text-center">
                                <h5>{{ __('PET_DATA') }}</h5>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal_old">{{ __('Return') }}</button>
                            <button type="submit" class="btn btn-primary"><i class="fas fa-save "></i> {{ __('SAVE') }}</button>
                        </div>
                    </form>
                </div>
...
@if(session('no_exist_customer')) {
<script>
    Swal.fire({
        icon: 'error',
        title: 'Lo siento!',
        text: 'Este cliente no se encuantra registrado',
        footer: 'Por favor, intente en "Nueva Historia"'
    })
</script>
}
@endif
